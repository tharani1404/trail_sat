...
db_config = {
    'host': 'sat2023-9055.8nk.gcp-asia-southeast1.cockroachlabs.cloud',
    'port': 26257,
    'user': 'tharanimahathisivara',
    'password': 'TCqh5WhluwdIwNH3WER5fg',
    'database': 'sat',
    'sslmode': 'verify-full',
...
# Function to get the database connection from Flask's application context
def get_db():
    if 'db' not in g:
...
def generate_video(duration_per_image, background_music=None, resolution=720):
    try:
        ...
        fps = 1/(duration_per_image)  # Frames per second

        # Convert duration_per_image to int if it's a string
        ...
        if background_music and os.path.isfile(background_music):  # Check if background_music is provided and is a valid file
        ...
        # Example: cursor.execute("INSERT INTO videos (path) VALUES (%s)", (video_path,))
        # Remember to commit changes if necessary: cursor.connection.commit()
